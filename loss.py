...
class MS_SSIM(torch.nn.Module):
    ...
    def _ssim(self, img1, img2, size_average = True):
        ...
        ssim_map = ((2*mu1_mu2 + C1)*V1)/((mu1_sq + mu2_sq + C1)*V2) # L * C * S
        mcs_map = V1 / V2 # C * S
        if size_average:
...
class MS_SSIM_L1(torch.nn.Module):
    def __init__(self, size_average = True, max_val = 255):
        """
    ...
    def ms_ssim(self, img1, img2, levels=5):
        ...
        for i in range(levels):
            ...
            img1 = filtered_im1
            img2 = filtered_im2
        value = (torch.prod(mcs[0:levels-1]**weight[0:levels-1])*(msssim[levels-1]**weight[levels-1])) # M = levels - 1
        return value

    def l1(self, img1, img2, size_average = True):
        l1_loss = F.conv2d(torch.abs(img1-img2), self.window, padding = self.window_size//2, groups = self.channel)
        if size_average:
            l1_loss = l1_loss.mean()
        return l1_loss

    def forward(self, img1, img2):
        ms_ssim_loss = self.ms_ssim(img1, img2)

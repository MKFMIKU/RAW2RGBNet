...
def make_model(opts):
    return EncoderDecoderNet(n_feats=32, n_blocks=8, n_resgroups=10)
...
class EncoderDecoderNet(nn.Module):
    ...
    def __build_model(self):
        ...
        self.uper = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.ConvTranspose2d(self.n_feats * 2, self.n_feats, kernel_size=4, stride=2, padding=1, bias=True)
        )
    ...
    def forward(self, x):
        ...
        fused_fea = torch.cat([local_fea, global_fea], dim=1)
